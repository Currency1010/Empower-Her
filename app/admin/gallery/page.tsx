...
export default function NewsUpdatesAdminPage() {
  ...
  return (
    <div className="space-y-8">
      <AdminPageHeader
        eyebrow="Content"
        title="News & Updates"
        description="Preview and manage published news updates. (UI-only mock data)"
      />
      ...
      >
          ...
          <p className="text-sm text-slate-600 leading-relaxed line-clamp-3">
            {post.content?.slice(0, 150) ?? ""}
                ...
                <div className="mt-auto flex items-center justify-between gap-3">
                  ...
                  <Button type="button" variant="ghost" className="text-primary hover:text-secondary" disabled aria-disabled>
                    Edit
                  </Button>
                </div>
              </div>
        ...
        {filtered.length === 0 ? <p className="mt-6 text-sm text-slate-600">No news matched your search.</p> : null}
      </AdminSurface>
        ...
        <DialogContent className="max-w-2xl">
          {viewing ? (
            <div className="space-y-4">
              ...
              <h2 className="font-heading text-2xl font-bold text-slate-900 leading-tight">
                {viewing.title}
              </h2>
              <p className="text-sm text-slate-600 leading-relaxed">{viewing.content}</p>
            </div>
          ) : null}
        </DialogContent>
      </Dialog>
